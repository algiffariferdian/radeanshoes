                ...
                <div class="surface-card-strong p-6">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-sm text-[var(--text-secondary)]">Dibuat pada
                                {{ optional($order->placed_at)->translatedFormat('d M Y, H:i') }} WIB
                            </p>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <span
                                class="rounded-full px-3 py-1 text-xs font-semibold {{ $order->order_status->badgeClasses() }}">{{ $order->order_status->label() }}</span>
                            <span
                                class="rounded-full px-3 py-1 text-xs font-semibold {{ $order->payment_status->badgeClasses() }}">{{ $order->payment_status->label() }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 grid gap-4 sm:grid-cols-3">
                        <div class="surface-soft p-4">
                            ...
                            <p class="mt-2 text-xl font-bold text-[var(--text-primary)]">
                        ...
                        <div class="surface-soft p-4">
                            ...
                            <p class="mt-2 text-base font-semibold text-[var(--text-primary)]">
                                {{ $order->tracking_number ?: 'Belum diinput admin' }}
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        @foreach ($order->items as $item)
                            <article class="flex gap-4 rounded-[1rem] border border-[var(--border-soft)] p-4">
                                <div class="h-20 w-20 overflow-hidden rounded-[0.9rem] bg-[var(--surface-soft)]">
                                    @if ($item->product?->primary_image_url)
                                        <img src="{{ $item->product->primary_image_url }}"
                                            alt="{{ $item->product_name_snapshot }}" class="h-full w-full object-cover">
                                    @else
                                        <div
                                            class="flex h-full items-center justify-center text-4xl font-semibold text-[var(--text-muted)]">
                                            !</div>
                                    @endif
                                </div>
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="mt-1 text-sm text-[var(--text-secondary)]">{{ $item->variant_size_snapshot }}
                                        / {{ $item->variant_color_snapshot }}</p>
                                    <div class="mt-3 flex flex-wrap items-center justify-between gap-3 text-sm">
                                        <span class="text-[var(--text-secondary)]">{{ $item->qty }} x
                                            Rp{{ number_format((float) $item->unit_price, 0, ',', '.') }}</span>
                                        <span
                                            class="font-semibold text-[var(--text-primary)]">Rp{{ number_format((float) $item->line_total, 0, ',', '.') }}</span>
                                    </div>

                                    @if ($order->order_status === \App\Support\Enums\OrderStatus::Completed)
                                        @if ($item->product)
                                            <div class="mt-4 border-t border-[var(--border-soft)] pt-4">
                                                <form method="POST" action="{{ route('products.reviews.store', $item->product) }}"
                                                    class="space-y-3">
                                                    @csrf
                                                    ...
                                                    <div class="flex flex-wrap items-center gap-2">
                                                        @for ($rating = 5; $rating >= 1; $rating--)
                                                            <label class="cursor-pointer" for="rating-{{ $item->id }}-{{ $rating }}">
                                                                <input id="rating-{{ $item->id }}-{{ $rating }}" type="radio"
                                                                    name="rating" value="{{ $rating }}" class="sr-only peer"
                                        ...
                                        @else
                                            <p class="mt-4 text-sm text-[var(--text-muted)]">Produk sudah tidak tersedia untuk
                                                ulasan.</p>
                                        @endif
                                    @else
                                        <p class="mt-4 text-sm text-[var(--text-muted)]">Ulasan tersedia setelah pesanan
                                            selesai.</p>
                                    @endif
                                </div>
                            </article>
                ...
                <div class="surface-card p-5">
                    ...
                    <div class="section-divider mt-4 pt-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="text-xl font-bold text-[var(--text-primary)]">Rp{{ number_format((float) $order->total_amount, 0, ',', '.') }}</span>
                        </div>
